...
NOTE: 藉由NetApp的《解決方案與解決ONTAP 方案與解決ONTAP 方案：解決方案與解決方案：解決方案與解決方案：解決方案與解決方案：請參閱 link:../rest/rbac_overview.html#summary-of-rbac-evolution["RBAC演進摘要"] 和 link:../rn/whats_new.html["更新功能：利用靜止API與自動化技術ONTAP"] 以取得更多資訊。
...
| 休息 | 其餘角色是ONTAP 以32個9.6加入、一般適用於透過ONTAP REST API存取的使用者。之後、他們也逐漸發展為支援CLI存取。 
