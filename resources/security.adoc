...
.啟用反勒索軟體
您可以控制自主勒索軟體防護（ARP）啟用功能的運作。這包括擷取和修改組態設定。此資源類型是在 ONTAP 9.18.1 中引入的。
...
.叢集網路安全
您可以檢索和更新叢集網路安全性配置，包括憑證。此資源類型是在ONTAP 9.18 中引入的。
...
.高可用性網路安全
您可以檢索和更新 HA 網路安全性設定。此資源類型是在ONTAP 9.18 中引入的。
...
.網路驗證
Web 驗證（WebAuthn）是一種基於公鑰加密技術的安全使用者身分驗證 Web 標準。透過 ONTAP，它支援透過 System Manager 和 ONTAP REST API 管理防釣魚的 MFA。此功能在 ONTAP 9.16 版本中添加，並在 ONTAP 9.19.1 版本中進行了更新。

.Whoami
提供端點以顯示已驗證使用者的詳細資料，包括使用者名稱、角色和權限。此資源類型是在 ONTAP 9.19.1 中引入的。
