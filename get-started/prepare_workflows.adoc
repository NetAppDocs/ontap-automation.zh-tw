...
某些 REST API 呼叫（例如使用 POST 或修補程式的呼叫）需要 JSON 輸入以供要求主體使用。JSON 輸入範例與捲曲範例分開呈現、以供清楚說明。您可以將 JSON 輸入範例與下列其中一種技術搭配使用。

.儲存至本機檔案
您可以將 JSON 範例複製到檔案中、並將其儲存在本機上。Curl 命令是使用來參照檔案 `--data` 參數、其值會以 a 表示檔案名稱 `@` 前置碼：

.在捲曲範例之後貼到終端機
首先、您需要將捲曲範例複製並貼到終端機殼層。然後編輯範例以完全移除 `--data` 最後的參數、並以取代 `--data-raw` 參數。最後、在 JSON 範例中複製並貼上、使其遵循 Curl 命令和更新的參數。
